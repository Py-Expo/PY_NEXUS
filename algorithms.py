...
def alg2_sequential(queues, argss, consecutive_frames, event):
    ...
    if not argss[0].plot_graph:
        ...
        f, ax = plt.subplots(figsize=(10, 6))  # Adjust the figure size here
        move_figure(f, 800, 100)
    window_names = [args.video if isinstance(args.video, str) else 'Cam '+str(args.video) for args in argss]
    [cv2.namedWindow(window_name, cv2.WINDOW_NORMAL) for window_name in window_names]  # Set window flags to allow resizing
    [cv2.resizeWindow(window_name, 800, 600) for window_name in window_names]  # Set initial window size
    while True:
